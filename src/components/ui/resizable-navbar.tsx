...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative main-top-nav z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full bg-transparent px-4 py-2 lg:flex dark:bg-transparent",
        visible && "bg-background/10 !border-blue-800/10 !border-[1px] dark:bg-neutral-950/80",
        className
      )}
    >
      {children}
        ...
        >
          {hovered === idx && (
            <motion.div
              ...
              className="absolute transition-colors transition-discrete inset-0 h-full w-full  rounded-full bg-white dark:bg-neutral-800"
            />
          )}
  ...
  return (
    <a href="#" className="relative z-20 mr-4 flex items-center  space-x-2 px-2 py-1 text-sm font-normal text-black">
      <img
        ...
        height={30}
      />
      {/* <span className="font-medium text-black dark:text-white">Agency</span> */}
    </a>
  );
};
